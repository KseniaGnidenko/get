...
ax.plot(t, volt, label = 'V(t)', color = 'yellow')

ax.set(title = ('Зависимость напряжения на конденсаторе от времени'),
    xlabel = ('Время, с'), ylabel = ('Напряжение, В'),
    xlim = (0, dur), ylim = (0, 3.5) )

ax.text(50, 1.5, 'Время зарядки: {:.2f} c'.format(ch_time), bbox=dict(facecolor='pink', alpha=0.5))
ax.text(50, 1, 'Время разрядки: {:.2f} c'.format(disch_time), bbox=dict(facecolor='pink', alpha=0.5))
